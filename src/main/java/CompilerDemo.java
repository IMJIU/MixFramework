...
public class CompilerDemo {
	 ...
	 * 返回JavaCompiler接口的一个实例 使用JavaCompiler最简单的方法是直接调用run()方法，run()方法是在Tool接口中实现的
	 */
	 ...
	 * 这个文件管理器提供了一种方法来完成普通文件的输入输出工作。 同时在一个 DiagnosticListener实例的帮助下报告编译的诊断信息。
	 * 后面将要用到的DiagnosticCollector类只是前面那个 listener的一个实现。
	 * 在确定什么东西是需要编译的之前，你需要一个文件管理器。
	 * 创建一个文件管理器需要两个基本的步骤： 创建一个DiagnosticCollector然后使用getStandardFileManager()方法向JavaCompiler申请文件管理器。
	...
	public static void compiler2() {
		...
		for (Diagnostic diagnostic : diagnostics.getDiagnostics()) {
			System.console().printf("Code: %s%n" + "Kind: %s%n" + "Position: %s%n" + "Start Position: %s%n" + "End Position: %s%n" + "Source: %s%n" + "Message:   %s%n", diagnostic.getCode(),
			        diagnostic.getKind(), diagnostic.getPosition(), diagnostic.getStartPosition(), diagnostic.getEndPosition(), diagnostic.getSource(), diagnostic.getMessage(null));
		}
		try {
			fileManager.close();
		} catch (IOException e) {
