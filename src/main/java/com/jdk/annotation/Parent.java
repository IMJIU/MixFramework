...
public class Parent<T> {
	...
	public List<SortableField> init() {
		...
		/**
		 * getClass().getGenericSuperclass()返回表示此 Class 所表示的实体（类、接口、基本类型或 void） 的直接超类的 Type(Class<T>泛型中的类型)，然后将其转换ParameterizedType。。 getActualTypeArguments()返回表示此类型实际类型参数的 Type 对象的数组。
		...
		if (this.entity != null) {
			...
			/**
			 * 返回类中所有字段，包括公共、保护、默认（包）访问和私有字段，但不包括继承的字段 entity.getFields();只返回对象所表示的类或接口的所有可访问公共字段 在class中getDeclared**()方法返回的都是所有访问权限的字段、方法等； 可看API
			 * */
			Field[] fields = entity.getDeclaredFields();
			//
			for (Field f : fields) {
				// 获取字段中包含fieldMeta的注解
				FieldMapper meta = f.getAnnotation(FieldMapper.class);
				if (meta != null) {
			...
			for (Method m : methods) {
				FieldMapper meta = m.getAnnotation(FieldMapper.class);
				if (meta != null) {
