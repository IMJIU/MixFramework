...
//		System.out.println(stringify(abyte0));
	...
	private static String stringify(byte abyte0[]) {
		...
		for (int i = 0; i < abyte0.length; i++) {
			...
			int k = abyte0[i] & 0xf;
			stringbuffer.append(new Character((char) (j <= 9 ? 48 + j
					: (97 + j) - 10)));
			stringbuffer.append(new Character((char) (k <= 9 ? 48 + k
					: (97 + k) - 10)));
		}
